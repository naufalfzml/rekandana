<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto px-3 sm:px-6 lg:px-8">
            <div class="glass-card overflow-visible sm:overflow-hidden sm:rounded-xl">
                <div class="p-4 sm:p-6 md:p-8 text-gray-100">
                    <form action="{{ route('mahasiswa.proposals.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5 sm:space-y-6 pb-4 sm:pb-0" onsubmit="return true;">
                        @csrf

                        <div>
                            <x-input-label for="title" value="Judul Proposal/Acara" />
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title')" required placeholder="Masukkan judul proposal atau acara" />
                        ...
                        <div>
                            ...
                            <x-text-input id="penyelenggara" name="penyelenggara" type="text" class="mt-1 block w-full" :value="old('penyelenggara')" required placeholder="Nama organisasi/komunitas penyelenggara" />
                            <x-input-error class="mt-2" :messages="$errors->get('tanggal_acara')" />
                        </div>

                        <div>
                            <x-input-label for="description" value="Deskripsi Singkat" />
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full textarea-dark rounded-md shadow-sm" placeholder="Tuliskan ringkasan acara, tujuan, dan manfaat">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <x-text-input id="link_sosmed" name="link_sosmed" type="text" class="mt-1 block w-full" :value="old('link_sosmed')" placeholder="Contoh: https://instagram.com/acara_kamu" />
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button class="w-full sm:w-auto">Ajukan Proposal</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
