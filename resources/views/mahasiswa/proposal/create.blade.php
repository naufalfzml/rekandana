        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="glass-card overflow-hidden rounded-lg sm:rounded-xl">
                <div class="p-4 sm:p-6 md:p-8 text-gray-100">
                    <form action="{{ route('mahasiswa.proposals.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
                        @csrf

                        <div>
                            <x-input-label for="title" value="Judul Proposal/Acara" />
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title')" placeholder="Contoh: Workshop Teknologi AI untuk Mahasiswa" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="penyelenggara" name="penyelenggara" type="text" class="mt-1 block w-full" :value="old('penyelenggara')" placeholder="Contoh: Himpunan Mahasiswa Teknik Informatika" required />
                            <x-input-error class="mt-2" :messages="$errors->get('tanggal_acara')" />
                        </div>

                        <div>
                            <x-input-label for="description" value="Deskripsi Singkat" />
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full textarea-dark rounded-md shadow-sm text-sm sm:text-base" placeholder="Jelaskan tujuan, manfaat, dan target peserta acara Anda...">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs sm:text-sm text-gray-400">Hanya file PDF yang diterima. Pastikan ukuran file maksimal 5MB.</p>
                        ...
                        <div>
                            ...
                            @if(auth()->user()->direct_proposal_quota > 0)
                                <div class="mb-2 p-2 sm:p-3 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                                ...
                                <select id="target_sponsor_id" name="target_sponsor_id" class="mt-1 block w-full select-dark rounded-md shadow-sm text-sm sm:text-base">
                                ...
                                <p class="mt-1 text-xs sm:text-sm text-gray-400">Jika tidak dipilih, proposal akan masuk ke daftar umum.</p>
                            @else
                                <div class="p-3 sm:p-4 bg-red-900/20 border border-red-500/30 rounded-lg">
                                    <p class="text-xs sm:text-sm text-red-300 mb-2">
                        ...
                        <div class="flex items-center gap-2 sm:gap-4 pt-2">
                            <x-primary-button class="w-full sm:w-auto text-sm sm:text-base px-4 sm:px-6 py-2 sm:py-3">Ajukan Proposal</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
