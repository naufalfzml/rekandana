<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header Section -->
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg p-4 sm:p-6 mb-6 sm:mb-8 text-white">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                        <h3 class="text-xl sm:text-2xl font-bold mb-1 sm:mb-2">Proposal Langsung</h3>
                        <p class="text-sm sm:text-base text-blue-100">Proposal yang dikirim khusus untuk perusahaan Anda</p>
                    </div>
                    <div class="text-left sm:text-right">
                        <div class="text-2xl sm:text-3xl font-bold">{{ $directProposals->count() }}</div>
                        <div class="text-blue-100 text-xs sm:text-sm">Total Proposal</div>
                    </div>
                </div>
            </div>

            <!-- Filters and Search -->
            <div class="glass-card neon-border overflow-hidden rounded-xl mb-4 sm:mb-6">
                <div class="p-4 sm:p-6">
                    <div class="flex flex-col lg:flex-row gap-4">
                        ...
                        <div class="flex-1">
                            <div class="relative">
                                <input type="text" id="search-proposals"
                                       class="w-full px-3 py-2.5 pl-9 sm:px-4 sm:py-3 sm:pl-10 input-dark rounded-lg text-sm sm:text-base"
                                       placeholder="Cari berdasarkan judul, nama mahasiswa, atau universitas...">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-300 absolute left-2.5 top-3 sm:left-3 sm:top-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-end">
                            <select id="status-filter" class="px-3 py-2.5 sm:px-4 sm:py-3 select-dark rounded-lg w-full sm:w-auto text-sm sm:text-base">
                            ...
                            <select id="category-filter" class="px-3 py-2.5 sm:px-4 sm:py-3 select-dark rounded-lg w-full sm:w-auto text-sm sm:text-base">
                            ...
                            <button id="clear-filters" class="px-3 py-2.5 sm:px-4 sm:py-3 btn-ghost rounded-lg transition-colors duration-200 w-full sm:w-auto text-sm sm:text-base">
                                Reset
                            </button>
                        </div>
                    </div>
                ...
                @forelse($directProposals as $invitation)
                    @if($invitation->proposal && $invitation->proposal->user)
                        <div class="relative proposal-card"
                            ...
                            <!-- Main Card -->
                            <div class="glass-card overflow-hidden rounded-xl neon-border hover:shadow-lg transition-all duration-200">
                                    ...
                                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="flex items-center text-gray-300">
                                                <div class="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center mr-2 flex-shrink-0">
                                                    <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                                    </svg>
                                                </div>
                                                <div>
                                                    <p class="text-sm sm:text-base font-medium text-white">{{ $invitation->proposal->user->name }}</p>
                                                    <p class="text-xs sm:text-sm text-gray-400">{{ $invitation->proposal->user->university ?? 'Universitas tidak tersedia' }}</p>
                                                </div>
                                            </div>
                                        </div>

                                        <!-- Right: Action Buttons -->
                                        <div class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto lg:ml-6">
                                            @if(in_array($invitation->proposal->id, $dealedProposalIds))
                                                <a href="{{ route('sponsor.deals.index') }}" class="px-4 py-2 bg-green-600 text-white text-xs sm:text-sm font-semibold rounded-lg text-center">
                                                    ✓ Dealed
                                                </a>
                                            @elseif(($invitation->status ?? 'pending') === 'pending')
                                                <button class="mark-interested px-4 py-2 bg-green-600 text-white text-xs sm:text-sm font-semibold rounded-lg hover:bg-green-700 transition-colors duration-200"
                                                        data-invitation-id="{{ $invitation->id }}"
                                                        data-proposal-id="{{ $invitation->proposal->id }}">
                                                    Simpan
                                                </button>
                                                <button class="mark-rejected px-4 py-2 bg-red-600 text-white text-xs sm:text-sm font-semibold rounded-lg hover:bg-red-700 transition-colors duration-200"
                                                        data-invitation-id="{{ $invitation->id }}">
                                                    Tolak
                                                </button>
                                            @elseif(($invitation->status ?? '') === 'interested')
                                                <form action="{{ route('sponsor.deals.initiate', $invitation->proposal) }}" method="POST" class="w-full sm:w-auto">
                                                    @csrf
                                                    <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-xs sm:text-sm font-semibold rounded-lg hover:from-emerald-600 hover:to-teal-700 transition-all duration-200 flex items-center justify-center">
                                ...
                                <div class="relative -mt-2">
                                    <button @click="expanded = !expanded"
                                            class="w-full py-2.5 sm:py-3 bg-gradient-to-b from-white/5 to-white/10 hover:from-white/10 hover:to-white/15 transition-all duration-200 flex items-center justify-center rounded-b-xl border-t border-white/10">
                                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-300 transition-transform duration-300"
                            ...
                            <div x-show="expanded"
                                ...
                                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                                    ...
                                    <!-- Description -->
                                    <div>
                                        <h4 class="text-base sm:text-lg font-bold text-white mb-2 sm:mb-3">Deskripsi Proposal</h4>
                                        <div class="bg-white/5 p-3 sm:p-4 rounded-lg border border-white/10">
                                            <p class="text-gray-300 text-xs sm:text-sm whitespace-pre-line">{{ $invitation->proposal->description }}</p>
                                        </div>
                                    </div>

                                    <!-- Funding Goal -->
                                    <div class="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 p-4 sm:p-6 rounded-lg border border-indigo-500/30">
                                        <p class="text-xs sm:text-sm font-bold text-indigo-300 mb-1 sm:mb-2">Target Pendanaan</p>
                                        <p class="text-2xl sm:text-3xl font-extrabold text-white">Rp {{ number_format($invitation->proposal->funding_goal, 0, ',', '.') }}</p>
                                    </div>
                                    ...
                                    <div>
                                        <a href="{{ asset('storage/' . $invitation->proposal->file_path) }}"
                                           target="_blank"
                                           class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white text-sm sm:text-base font-semibold rounded-lg hover:bg-blue-700 transition-colors duration-200 w-full sm:w-auto justify-center">
                ...
                @empty
                    ...
                    <div class="glass-card overflow-hidden rounded-xl">
                        <div class="p-8 sm:p-12 text-center">
                            <svg class="w-16 h-16 sm:w-24 sm:h-24 text-gray-300 mx-auto mb-4 sm:mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <h3 class="text-lg sm:text-xl font-semibold text-white mb-2 px-4">Belum ada proposal langsung</h3>
                            <p class="text-sm sm:text-base text-gray-300 mb-4 sm:mb-6 px-4">
                                Belum ada mahasiswa yang mengirim proposal langsung ke perusahaan Anda.
                            </p>
                            <a href="{{ route('sponsor.proposals.index') }}"
                               class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 btn-gradient text-white text-sm sm:text-base font-semibold rounded-lg transition-colors duration-200">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
