        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="glass-card overflow-hidden rounded-lg sm:rounded-xl">
                <div class="p-4 sm:p-6 md:p-8 text-gray-100">
                    <form method="POST" action="{{ route('admin.referral-codes.update', $referralCode) }}" class="space-y-4 sm:space-y-5">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="code" value="Kode Referral" />
                            <x-text-input id="code" class="block mt-1 w-full text-sm sm:text-base" type="text" name="code" :value="old('code', $referralCode->code)" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="max_uses" class="block mt-1 w-full text-sm sm:text-base" type="number" name="max_uses" :value="old('max_uses', $referralCode->max_uses)" min="1" placeholder="Kosongkan untuk unlimited" />
                        ...
                        <div>
                            ...
                            <x-text-input id="expires_at" class="block mt-1 w-full text-sm sm:text-base" type="datetime-local" name="expires_at" :value="old('expires_at', $referralCode->expires_at ? $referralCode->expires_at->format('Y-m-d\TH:i') : '')" />
                            <p class="text-xs sm:text-sm text-gray-400 mt-1">Biarkan kosong jika kode tidak memiliki masa kadaluarsa</p>
                            <x-input-error :messages="$errors->get('expires_at')" class="mt-2" />
                        </div>

                        <div class="p-3 sm:p-4 bg-slate-800/50 border border-white/10 rounded-lg">
                            <p class="text-xs sm:text-sm text-gray-300 font-semibold mb-1">Statistik:</p>
                            <p class="text-xs sm:text-sm text-gray-300">Sudah digunakan oleh: <strong class="text-white">{{ $referralCode->users_count ?? 0 }}</strong> sponsor</p>
                        </div>
